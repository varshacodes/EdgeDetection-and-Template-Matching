...
def convolve2d(img, kernel):
    ...
    kSize = len(kernel) -1
    #pads the image based on kernel size
    padSize = (int)(kSize/2)
    img = utils.zero_pad(img,padSize,padSize)
    ConvolvedImg = [] #convolved Img  to be stored
    m=0
    n=0
    sum=0
...
def edge_magnitude(edge_x, edge_y):
    ...
    for i in range(0,len(edge_x)):
        for j in range(0,len(edge_x[0])):
            edge_mag[i][j] = np.sqrt((edge_x[i][j]*edge_x[i][j])+(edge_y[i][j]*edge_y[i][j]));
    #Different normalization applied for edge_magnitude 
    #to ensure that max pixel value is 1     
    # TODO: implement this function.
    #raise NotImplementedError
    return edge_mag

def normalizeEdgeMag(edge_mag):
    #used different normalization for edge Mag to make sure the maximum
    #pixel value is 1
    # Also the change in normalization helped in achieving output 
    #closer to one that was desired
    min = 1
    max = 0
    for i in range(0,len(edge_mag)):
        for j in range(0,len(edge_mag[0])):
            if edge_mag[i][j] < min:
                min = edge_mag[i][j]
            if edge_mag[i][j] > max:
                max = edge_mag[i][j]
...
def main():
    ...
    img_edge_x = detect_edges(img, kernel_x, True)
    ...
    img_edge_y = detect_edges(img, kernel_y, True)
    ...
    img_edges = normalizeEdgeMag(edge_magnitude(img_edge_x, img_edge_y))
